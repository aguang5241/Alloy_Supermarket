...
from PyQt5.QtWidgets import QWidget, QApplication, QVBoxLayout, QLabel, QTabWidget, QGridLayout, QGroupBox, QPushButton, QDoubleSpinBox, QHBoxLayout, QLineEdit, QSpinBox,QTableView,QScrollBar,QMessageBox
from PyQt5.QtGui import QPixmap, QIcon, QFont, QIntValidator, QDoubleValidator,QStandardItemModel,QStandardItem
from PyQt5.QtCore import Qt
from PyQt5.QtSql import QSqlDatabase, QSqlQuery


class AlloySupermarket(QWidget):
    def __init__(self):
        ...
        self.db.setDatabaseName('res/AsDB.db')

    def initUI(self):
        layout = QVBoxLayout()
        la_declear = QLabel(
            'Alloy Supermarket v1.0 \n  Contact US: lijun.zhang@csu.edu.cn')
        la_declear.setAlignment(Qt.AlignCenter)
        self.resize(800, 800)
        self.setWindowTitle('Alloy Supermarket')
        self.setWindowIcon(QIcon('res/as.ico'))
        self.tabWedget = QTabWidget()
        ...
        self.tab2 = QWidget()
        self.tab3 = QWidget()
        self.tabWedget.addTab(self.tab1, 'Concentrations --> Performance')
        self.tabWedget.addTab(self.tab2, 'Performance --> Concentrations')
        self.tabWedget.addTab(self.tab3, '         Contour Map          ')

        layout.addWidget(self.tabWedget)
        layout.addWidget(la_declear)
    ...
    def tab1UI(self):
        ...
        self.input_Mg.setSingleStep(0.01)
    ...
    def tab2UI(self):
        ...
        self.model.setHorizontalHeaderLabels(
            ['Al (wt.%)', 'Si (wt.%)', 'Mg (wt.%)', 'Sc (wt.%)', 'UTS (MPa)', 'YS (MPa)', 'EL (%)'])
    ...
    def tab3UI(self):
        ...
        img_label = QLabel()
        img = QPixmap('res/CM.jpg')
        img_label.setAlignment(Qt.AlignCenter)
        img_ = img.scaled(800, 800, Qt.KeepAspectRatio)

        img_label.setPixmap(img_)
    ...
    def tab1_predict(self):
        ...
        # 数据库操作
        if not self.db.open():
            QMessageBox.critical(self, 'Critical', 'Can Not Open Database',
                                 QMessageBox.Yes | QMessageBox.No, QMessageBox.Yes)
            # print('not open')
        else:
            query = QSqlQuery()
            sql = 'select Al, Sc, UTS, YS, EL from Al_Si_Mg_Sc where Si = %.2f and Mg = %.2f' % (
                Si, Mg)
            query.prepare(sql)
            if not query.exec_():
                QMessageBox.critical(self, 'Critical', query.lastError(
                ), QMessageBox.Yes | QMessageBox.No, QMessageBox.Yes)
            else:
                while query.next():
                    Al = query.value(0)
    ...
    def tab2_predict(self):
        ...
        if (self.input_UTS.text() == '') | (self.input_YS.text() == '') | (self.input_EL.text() == ''):
            QMessageBox.critical(self, 'Critical', 'Please Input Data',
                                 QMessageBox.Yes | QMessageBox.No, QMessageBox.Yes)
        else:
            ...
            if not self.db.open():
                ...
                query = QSqlQuery()
                sql = 'select Al, Si, Mg, Sc, UTS, YS, EL from Al_Si_Mg_Sc where (UTS between %.2f and %.2f) and (YS between %.2f and %.2f) and (EL between %.2f and %.2f)' % (
                    UTS-UTS_e, UTS+UTS_e, YS-YS_e, YS+YS_e, EL-EL_e, EL+EL_e)
                query.prepare(sql)
                isFound = False
                if not query.exec_():
                    QMessageBox.critical(self, 'Critical', query.lastError(
                ...
                if not isFound:
                    QMessageBox.information(
                        self, 'Message', 'Found No Data', QMessageBox.Yes | QMessageBox.No, QMessageBox.Yes)
